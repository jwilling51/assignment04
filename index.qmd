...
top_cities <- urban_areas |>
  filter(NAME10 %in% c("Laredo, TX", "San Diego, CA", "Miami, FL", 
                       "New Orleans, LA", "San Francisco--Oakland, CA")) |>
...
n <- inadmissibles_2014 |>
  ...
  arrange(desc(n)) |>
  filter(field_office_name %in% c("LAREDO", "SAN DIEGO", "MIAMI", "NEW ORLEANS",
                                  "SAN FRANCISCO")) |>
...
ggplot() + 
  ...
  geom_sf(data = points, alpha = 0.5, aes(size = n, color = field_office_name),
          show.legend = FALSE) + 
  geom_sf_text(data = points, aes(label = field_office_name), 
               fontface = "bold",
               nudge_x = ifelse(points$field_office_name == "San Francisco, 
                                CA", 2.0, 0),
               nudge_y = 2.0) +
  ...
  scale_size_area(max_size = 15) +
  theme_void() +
  labs(title = "Top 5 Customs and Border Protection Field Offices 
